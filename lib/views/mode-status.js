...
class ModeStatus extends HTMLDivElement{
	...
	initialise(statusBar){
		...
		/** Pull existing mode from active file */
		const ed   = atom.workspace.getActiveTextEditor();
		const path = ed ? ed.getPath() : null;
		if(ed && Modes.files[path])
			this.setTo(Modes.files[path]);
	}
	...
	refresh(){
		...
		/** Relevant */
			...
			const mode = this.currentMode || Modes.default;
			this.style.display = "";
			this.currentEditor = ed;
			this.labelNode.textContent = mode.label;
	...
	/**
	 * Update the displayed text when the current file's mode changes.
	 *
	 * @param {Object} mode
	 * @private
	 */
